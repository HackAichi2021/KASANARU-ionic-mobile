...
export class CheckPage implements OnInit {
  ...
  postLendAndBorrow(x: number): void {
    ...
    this.httpclient
      ...
      .subscribe((res) => {
        console.log(res);
        this.store.setYourInfo(new YourInfo(res.user_id, this.parseLendAndBorrow(res.Info), this.parseFavorite(res.Favorite), res.username, res.Info.latitude, res.Info.longitude, res.Info.email));
